...
public:
  ...
  void onInit() override {
    ...
    odom_child_frame_id = private_nh.param<std::string>("odom_child_frame_id", "base");
    ...
    if (use_imu) {
      ...
      imu_sub = mt_nh.subscribe("filtered_imu", 256, &HdlLocalizationNodelet::imu_callback, this);
    }
    points_sub = mt_nh.subscribe("/velodyne_points", 5, &HdlLocalizationNodelet::points_callback, this);
    globalmap_sub = nh.subscribe("/globalmap", 1, &HdlLocalizationNodelet::globalmap_callback, this);
    initialpose_sub = nh.subscribe("/initialpose", 8, &HdlLocalizationNodelet::initialpose_callback, this);

    pose_pub = nh.advertise<nav_msgs::Odometry>("/odom_align", 5, false);
...
//SetInputSource
    ...
    Eigen::Matrix4f before = pose_estimator->matrix(); /*[0:3, 0:3]: Ma trận quay (rotation)
                                                         [0:3, 3]  : Vị trí (x, y, z)*/
    // predict
    if (!use_imu) {
      pose_estimator->predict(stamp);
      ROS_INFO("pose_estimator value: %f %f %f %f %f %f %f", before(0, 3), before(1, 3), before(2, 3), before(0, 0), before(1, 0), before(2, 0), before(3, 0));
    } else {
      std::lock_guard<std::mutex> lock(imu_data_mutex);
      ...
      for (imu_iter; imu_iter != imu_data.end(); imu_iter++) {
        ...
        // trích xuât gia tốc & vận tốc góc
        const auto& acc = (*imu_iter)->linear_acceleration;
        const auto& gyro = (*imu_iter)->angular_velocity;
        // dùng để đảo chiều sensor(nếu cần)
        ...
        Eigen::Matrix4f mat = pose_estimator->matrix();
        Eigen::Vector3f position = mat.block<3, 1>(0, 3);  // x, y, z
        Eigen::Matrix3f rot = mat.block<3, 3>(0, 0);       // rotation
        Eigen::Vector3f rpy = rot.eulerAngles(0, 1, 2);    // roll, pitch, yaw

        ROS_INFO_STREAM("Pose after predict :Pos(x, y, z): [" << position.x() << ", " << position.y() << ", " << position.z());
      }
      imu_data.erase(imu_data.begin(), imu_iter);
    }

    // odometry-based prediction
    // Nội suy chuyển động 2 thời điểm trong frame
    ros::Time last_correction_time = pose_estimator->last_correction_time();
    ROS_INFO("last_correction_time: %f", last_correction_time.toSec());
    ROS_INFO("enable_robot_odometry_prediction: %d", private_nh.param<bool>("enable_robot_odometry_prediction", false));
    if (private_nh.param<bool>("enable_robot_odometry_prediction", false) && !last_correction_time.isZero()) {
      ...
      if (tf_buffer.canTransform(odom_child_frame_id, last_correction_time, odom_child_frame_id, stamp, robot_odom_frame_id, ros::Duration(0.1))) {
        ...
        ROS_INFO("Transform using time 0");
      } else {
        ROS_ERROR("odom_delta false");
      }

      if (odom_delta.header.stamp.isZero()) {
        NODELET_WARN_STREAM("failed to look up transform between KKK" << cloud->header.frame_id << " and " << robot_odom_frame_id);
      } else {
        Eigen::Isometry3d delta = tf2::transformToEigen(odom_delta);
        // Dự đoán pose mới bằng odom_delta
        pose_estimator->predict_odom(delta.cast<float>().matrix());

        Eigen::Matrix4f predicted_pose = pose_estimator->matrix();
        Eigen::Vector3f pos = predicted_pose.block<3, 1>(0, 3);
        Eigen::Matrix3f rot = predicted_pose.block<3, 3>(0, 0);
        Eigen::Vector3f rpy = rot.eulerAngles(0, 1, 2);  // Roll, Pitch, Yaw

        ROS_INFO_STREAM("Point predict form IMU (x, y, z): [" << pos.x() << ", " << pos.y() << ", " << pos.z() << "], Yaw: " << rpy[2] * 180.0 / M_PI << "°");
      }
    } else {
      ROS_INFO("no odom_delta found");
    }

    // Hiệu chỉnh (correct) pose bằng pointcloud
    auto aligned = pose_estimator->correct(stamp, filtered);

    Eigen::Matrix4f corrected_pose = pose_estimator->matrix();
    Eigen::Vector3f pos_c = corrected_pose.block<3, 1>(0, 3);
    Eigen::Vector3f rpy_c = corrected_pose.block<3, 3>(0, 0).eulerAngles(0, 1, 2);

    std::cout << "Align odom (x, y, z): [" << pos_c.x() << ", " << pos_c.y() << ", " << pos_c.z() << "], Yaw: " << (rpy_c[2] * 180.0 / M_PI) << "°" << std::endl;
    pos_c.z() = 0.0;
    if (aligned_pub.getNumSubscribers()) {
      ...
      // Lấy vị trí từ ma trận pose
      pose_stamped.pose.position.x = corrected_pose(0, 3);
      pose_stamped.pose.position.y = corrected_pose(1, 3);
      pose_stamped.pose.position.z = 0.0;

      Eigen::Matrix3f rot_c = corrected_pose.block<3, 3>(0, 0);
      Eigen::Quaternionf quat(rot_c);

      // Kiểm tra và chuẩn hóa quaternion
      ...
      // Eigen::Quaternionf quat(rot_c.normalized());
      pose_stamped.pose.orientation.x = quat.x();
      pose_stamped.pose.orientation.y = quat.y();
      pose_stamped.pose.orientation.z = quat.z();
      pose_stamped.pose.orientation.w = quat.w();

      // Cập nhật path  path_.header.frame_id = "map";
  ...
  void initialpose_callback(const geometry_msgs::PoseWithCovarianceStampedConstPtr& pose_msg) {
    ...
    ROS_INFO("pose_init : %f %f %f", p.x, p.y, p.z);
    pose_estimator.reset(new hdl_localization::PoseEstimator(
      registration,
  ...
  void publish_odometry(const ros::Time& stamp, const Eigen::Matrix4f& pose) {
    ...
    ROS_INFO("pose odometry: %f %f %f", odom.pose.pose.position.x, odom.pose.pose.position.y, odom.pose.pose.position.z);
    pose_pub.publish(odom);
  }

  /**
   * @brief publish scan matching status information
   * @function Determine if the matching results are convergent
   *           Matching error calculation (distance between matching points)
   *           Lower count ratio(matched number /total valid number)
   *           Outputs the expected error change between the before and after quantify(prediction erro)
   */
  void publish_scan_matching_status(const std_msgs::Header& header, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned) {
    ...
    // A point is considered “inlier” if it has a compatible object in the target cloud within 0.5m.
    const double max_correspondence_dist = private_nh.param<double>("status_max_correspondence_dist", 0.5);
    // Evaluate distances less than 25.0m
    const double max_valid_point_dist = private_nh.param<double>("status_max_valid_point_dist", 25.0);

    // calculate errors
    int num_inliers = 0;  // model fit scores
    int num_valid_points = 0;
    std::vector<int> k_indices;
    std::vector<float> k_sq_dists;  // squared distance
    ...
    for (int i = 0; i < aligned->size(); i++) {
      ...
      num_valid_points++;
      // Find nearest neighbor point in target point cloud(pointcloud map))
      registration->getSearchMethodTarget()->nearestKSearch(pt, 1, k_indices, k_sq_dists);
      // If the distance point from map to point livox is less than max_correspondence_dist(0.5m), it is considered a match
    ...
    double actual_distance_error = std::sqrt(status.matching_error);
    status.matching_error = actual_distance_error;
    ROS_INFO("Distance error: %f (m)", status.matching_error);
    status.inlier_fraction = static_cast<float>(num_inliers) / std::max(1, num_valid_points);

    // convert to PCL sang ros
